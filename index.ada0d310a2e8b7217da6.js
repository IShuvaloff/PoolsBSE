...
function elementNavMenuItem(id, showShortNames, className) {
  const item = getMenuItem(id);
  if (!item) return;
  return el(`li.${className}__menu-item`, [showShortNames ? item.captionShort : item.caption]);
}
...
var close_btn_symbol = new (browser_symbol_default())({
  ...
  "use": "close-btn-usage",
  "viewBox": "0 0 24 24",
  "content": "<symbol xmlns=\"http://www.w3.org/2000/svg\" fill=\"none\" viewBox=\"0 0 24 24\" id=\"close-btn\"><path stroke=\"#000\" stroke-width=\"3\" d=\"M22 2 12 12M2 22l10-10m0 0L2.833 2.833 22 22\" /></symbol>"
});
...
function panelBurgerMenu() {
  const container = el(`.${CLASS_NAME}-container`, [el(`.${CLASS_NAME}`, [elementCloseBtn(CLASS_NAME), elementLogo(CLASS_NAME), panelNavMenu(CLASS_NAME, true), panelSocials(CLASS_NAME), elementOrderCall(CLASS_NAME)])]);
