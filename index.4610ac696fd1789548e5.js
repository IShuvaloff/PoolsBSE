...
function elementOrderCall(className) {
  const btn = el(`button.btn.btn-call-order.${className}--btn`, 'Заказать звонок');
...
function elementServiceCard(id, checked = false) {
  ...
  const btn = el(`button.btn.service__btn.service__btn--${item.id}`, 'Подробнее');
